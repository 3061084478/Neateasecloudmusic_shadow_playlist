...
class SongSharePage(QtWidgets.QWidget):
    def __init__(self, controller: AppController, thread_pool: QtCore.QThreadPool, parent: QtWidgets.QWidget | None = None):
        ...
        title_box.addWidget(make_label("歌曲分享", "pageTitle", False))
        ...
        self.keyword_edit.setPlaceholderText("歌曲名 / 歌手 / 发送方关键词")
        ...
        self.query_button = make_button("查询歌曲分享", "primaryButton")
        ...
        result_header.addWidget(make_label("结果区", "sectionTitle", False))
        self.summary_badge = make_label("0 条结果", "statusBadge", False)
        result_header.addStretch(1)
        result_header.addWidget(self.summary_badge)
        layout.addLayout(result_header)

        self.result_container = QtWidgets.QVBoxLayout()
        self.result_container.setSpacing(12)
        layout.addLayout(self.result_container)
        layout.addStretch(1)
    ...
    def _collect_filters(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "start_date": self.start_edit.date().toString("yyyy-MM-dd") if query_mode == "range" else None,
            "end_date": self.end_edit.date().toString("yyyy-MM-dd") if query_mode == "range" else None,
    ...
    def run_query(self) -> None:
        ...
        run_async(
            self.thread_pool,
            self.controller.query_song_shares,
    ...
    def render_results(self, items: List[Dict[str, Any]], summary: Dict[str, Any]) -> None:
        ...
        if not items:
            self.result_container.addWidget(make_label("当前没有命中歌曲分享结果。", "mutedText"))
            return
        for item in items:
            ...
            artist = item.get("artist_name") or "未知歌手"
            title_label = make_label(
                f"{item.get('song_name') or '未知歌曲'} - {artist}",
            ...
            bubble_layout.addWidget(
                make_label(item.get("msg_time_str") or "-", "messageSelfMeta" if is_self else "messageFriendMeta", False)
            )
            sender = "好友" if item.get("direction") == "friend" else "我方"
            meta_label = make_label(sender, "messageSelfMeta" if is_self else "messageFriendMeta", True)
            meta_label.setWordWrap(True)
            bubble_layout.addWidget(meta_label)
            source_label = make_label(
                f"来源说明：msg_id={item.get('msg_id') or '-'}",
            ...
            if is_self:
                ...
                row_layout.addStretch(1)
            self.result_container.addWidget(row)

    def render_error(self, message: str) -> None:
        self._clear_result_layout()
        self.summary_badge.setText("错误状态")
        self.result_container.addWidget(make_label(message or "查询失败。", "mutedText"))
